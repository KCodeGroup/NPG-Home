...
export default function MainButton({
  ...
  const buttonStyles = {
    ...
    "&:hover": {
      backgroundColor:
        variant === "contained" ? "#a67145" : "rgba(189, 131, 83, 0.1)",
  ...
  if (href) {
    return (
      <Button component={Link} href={href} variant={variant} sx={buttonStyles}>
        {icon && (
  ...
  return (
    <Button onClick={onClick} variant={variant} sx={buttonStyles}>
      {icon && (
        <span style={{ display: "flex", alignItems: "center" }}>{icon}</span>
      )}
      {children}
